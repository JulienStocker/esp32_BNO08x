...
// No more global static imu pointer - using instance-specific pointers
...
{
    bno08x_sh2_hal_t* hal_wrapper = (bno08x_sh2_hal_t*)self;
    BNO08x* imu = hal_wrapper->instance;

    spi_wait_for_int(imu);
...
void BNO08xSH2HAL::hal_cb(void* cookie, sh2_AsyncEvent_t* pEvent)
{
    if (pEvent->eventId == SH2_RESET) {
        BNO08x* imu_ptr = (BNO08x*)cookie;
        xEventGroupSetBits(imu_ptr->sync_ctx.evt_grp_task, BNO08xPrivateTypes::EVT_GRP_BNO08x_TASK_RESET_OCCURRED);
    }
}
...
{
    BNO08x* imu_ptr = (BNO08x*)cookie;
    xQueueSend(imu_ptr->queue_rx_sensor_event, event, 0);
}
 ...
 * @return void, nothing to return
 */
void BNO08xSH2HAL::hardware_reset(BNO08x* imu)
{
    imu->toggle_reset();
}
 ...
 */
bool BNO08xSH2HAL::spi_wait_for_int(BNO08x* imu)
{
 ...
 */
uint16_t BNO08xSH2HAL::spi_read_sh2_packet_header(BNO08x* imu, uint8_t* pBuffer)
{
 ...
 */
int BNO08xSH2HAL::spi_read_sh2_packet_body(BNO08x* imu, uint8_t* pBuffer, uint16_t packet_sz)
{
